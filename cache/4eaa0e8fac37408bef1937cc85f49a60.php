...
<div class="newblog left">
  ...
  <?php foreach ($message as $value):?>
   <h2><?=$value['title'];?></h2>
    ...
    <div class="page"><a title="Total record"><b><?=$total;?></b></a><a href="index.php?m=search&a=cha&page=<?=$allPage['first'];?>"><b>首页</b></a><a href="index.php?m=search&a=cha&page=<?=$allPage['prev'];?>"><</a><a href="index.php?m=search&a=cha&page=<?=$allPage['next'];?>">></a><a href="index.php?m=search&a=cha&page=<?=$allPage['end'];?>">尾页</a></div>
</div>
<aside class="right">
 <!-- <form action="index.php?m=details&a=find" method="post"> -->
   <div class="rnav" style="width:200px;height:200px;">
    <?php if(!empty($udertype)):?>
     <a href="index.php?m=update&a=update"><img src="<?=$pic;?>" style="width:200px;height:200px;"></a>
    <?php else: ?>
...
<h3>
    ...
    <?php foreach ($article as $val):?>
      <li><a href="index.php?m=details&a=reply&id=<?=$val['id'];?>" title="Column 三栏布局 个人网站模板" target="_blank"><?=$val['title'];?></a></li>
    <?php endforeach;?>
    <?php endif;?>
    </ul>
    <h3 class="ph">
    ...
    <?php foreach ($hits as $vel):?>
      <li><a href="index.php?m=details&a=reply&id=<?=$vel['id'];?>" title="Column 三栏布局 个人网站模板" target="_blank"><?=$vel['title'];?></a></li>
      <?php endforeach;?>
      <?php endif;?>
    </ul>
    </div>
